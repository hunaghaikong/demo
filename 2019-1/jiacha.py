...
def plan1(hsi, hsi_dict):
    ...
    list_hsi.sort()
    zuihou = {list_hsi[i] for i in range(len(list_hsi) - 1) if list_hsi[i][:6] != list_hsi[i + 1][:6]}
...
def main():
    ...
    hsi_day = pd.read_csv('HSI_day.csv', encoding='gbk')

    # 当月合约与日期为键，5分位、10分位、50分位、90分位、95分位为值保存字典
    hsi_dict = {f'{i[3]}': [i[7], i[8], i[10], i[12], i[13]] for i in hsi_day.values}

    res = plan1(hsi, hsi_dict)
    res2 = []  # 总盈亏统计
