...
class HS:
    def __init__(self):
        self.SXF = {"HSIK8": 33.54, "MHIK8": 13.6}  # 13.6
        self.hy = {"HSIK8": 50, "MHIK8": 10}  # 10
    ...
    def datas(self):
        ...
        while 1:
            ...
            if data['jy'] != 0:
                ...
                jcbs = (data['wcds1'] + abs(data['jy'])) * self.SXF[msg[3]] * 2 / self.hy[msg[3]] / data['jy']
    ...
    def ray(self, df):
        ...
        while ind < len(df.values):
            ...
            jzcbs = (data['wcds'] + abs(data['jy'])) * self.SXF[msg[3]] * 2 / self.hy[msg[3]] / data['jy'] if data[
            ...
            jlr = round(data['all'] * self.hy[msg[3]] - self.SXF[msg[3]] * data['all_jy_add'], 2)  # 净利润
            ...
            if ind != is_ind or (ind == is_ind and data['jy'] == 0):
                res.append(
                    [msg[3], data['time'], msg[0], pri, data['jy'], yscb, cb, jcb, cbyl, data['pcyl_all'], data['all'],
                     pjyl,
                     huihuapj, zcb, jzcb, data['all'] * self.hy[msg[3]], jlr, jpjlr,
                     round(self.SXF[msg[3]] * data['all_jy_add'], 2), data['wcds'], data['dbs']])
                cbyl = 0
