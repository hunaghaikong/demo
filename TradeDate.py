...
def getDate(size=300,code='hkHSI'):
    ...
    '''
    s=requests.get('http://web.ifzq.gtimg.cn/appstock/app/kline/kline?_var=kline_dayqfq&param=%s,day,,,%s'%(code,size)).text
...
def get_min_data(code='HSIc1'):
    ...
    conn=MyUtil.get_conn('stock_data')
    cur=conn.cursor()
    cur.execute("SELECT DATETIME,OPEN,high,low,CLOSE,amount,vol FROM index_min WHERE CODE='%s' ORDER BY DATETIME"%code)
    mins=cur.fetchall()
    conn.close()
    mi=[]
...
def mysql_min():
    ...
    conn=MyUtil.get_conn('stock_data')
    cur = conn.cursor()
    cur.execute("SELECT DATE_FORMAT(datetime,'%Y-%m-%d'),COUNT(*) FROM index_min WHERE CODE='HSIc1' GROUP BY DATE_FORMAT(datetime,'%Y%m%d')")
    mins=cur.fetchall()
    conn.close()
